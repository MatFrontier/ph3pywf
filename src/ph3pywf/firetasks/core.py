...
class DisplacedStructuresAdderTask(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        if struct_unitcell is None:
            ...
            struct_unitcell = Structure.from_dict(
                doc["calcs_reversed"][0]["output"]["structure"]
            )
        ...
        if supercell_size_fc2 is not None:
            for i, structure in enumerate(struct_displaced_fc2):
                ...
                disp_id = f"{i:05d}"
                vis_dict["structure"] = structure.as_dict()  # update vis_static
                vis_static = vis_static.__class__.from_dict(
                    vis_dict
                )  # update vis_static
                #                 logger.info(f"Adder: After update: vis.structure.num_sites={vis_static.structure.num_sites}")
                fw = ForceSymmStaticFW(
...
class Phono3pyAnalysisToDb(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        supercell_size_fc3 = addertask_dict["user_settings"].get(
            "supercell_size_fc3", None
        )
        supercell_size_fc2 = addertask_dict["user_settings"].get(
        ...
        if is_nac:
            logger.info(
                f'PostAnalysis: Reading nac_params from file: "{born_filename}"'
            )
        ...
        write_yaml_from_dict(
            ph3py_dict["band_structure"], "band_structure.yaml"
        )  # FOR TESTING
        ...
        ph3py_dict["kappa_hdf5"] = insert_gridfs_dict(
            kappa_dict, mmdb.db, "ph3py_tasks_fs"
        )
        ...
        for filename in filenames:
            ph3py_dict[filename] = insert_gridfs_file(
                filename, mmdb.db, "ph3py_tasks_fs"
            )
...
class StoreStructureTask(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        struct_dict = {
            "calcs_reversed": [{"output": {"structure": None}}],
            "task_label": None,
        }
...
class Phono3pyMeshConvergenceToDb(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        for mesh in mesh_list:
            ...
            if "kappa-m{}{}{}.hdf5".format(*mesh) in os.listdir(os.getcwd()):
                logger.info(
                    "PostAnalysis: Deleting previous kappa-m{}{}{}.hdf5 file".format(
                        *mesh
                    )
                )
                os.remove("kappa-m{}{}{}.hdf5".format(*mesh))
    ...
    def run_task(self, fw_spec):
        ...
        for direction in range(3):
            for t in range(len(temperature)):
                logger.info(
                    f"Now fitting direction = {direction}, temperature = {temperature[t]}"
                )
                xdata = np.array(
                    [
                        conv_test_doc["convergence_test"][i]["mesh"][direction]
